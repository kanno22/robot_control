...
void CAN::CAN_init(int EposNodeID_R,int EposNodeID_L)
{    
...
/////////////////////右足
  while( Enable(EposNodeID_R) !=1)
  {
    sprintf(sbuf, "Node%d 初期化\r\n",EposNodeID_R);
    Serial.println(sbuf);    
  }

  sprintf(sbuf, "Disenable %d:%d",EposNodeID_R, Disenable(EposNodeID_R) );
  Serial.println(sbuf);
  
  sprintf(sbuf, "Clearfault %d:%d",EposNodeID_R,  Clearfault(EposNodeID_R) );
  Serial.println(sbuf);
  
  sprintf(sbuf, "Enable %d:%d",EposNodeID_R,  Enable(EposNodeID_R) );
  Serial.println(sbuf);

  Serial.println("EPOS_R 初期化完了");
/////////////////////左足

  while( Enable(EposNodeID_L) !=1)
  {
    sprintf(sbuf, "Node%d 初期化\r\n",EposNodeID_L);
    Serial.println(sbuf);    
  }

  sprintf(sbuf, "Disenable %d:%d",EposNodeID_L, Disenable(EposNodeID_L) );
  Serial.println(sbuf);
  
  sprintf(sbuf, "Clearfault %d:%d",EposNodeID_L,  Clearfault(EposNodeID_L) );
  Serial.println(sbuf);
  
  sprintf(sbuf, "Enable %d:%d",EposNodeID_L,  Enable(EposNodeID_L) );
  Serial.println(sbuf);
    
  Serial.println("EPOS_L 初期化完了");
}
